class TransactionController < ApplicationController
  ...
  def sr_live_leads
    ...
    if params[:from] == nil
      @telephony_calls = []
      @selected_executive = -1
      @from = DateTime.now
      @to = DateTime.now
    else
      if current_personnel.email == "ayush@thejaingroup.com"
        @selected_executive = params[:personnel_id]
      else
        @selected_executive = current_personnel.id
      end
      @from = params[:from].to_datetime
      @to = params[:to].to_datetime
      @telephony_calls = TelephonyCall.where(agent_number: Personnel.find(@selected_executive).mobile, fresh: nil, untagged: true).where('created_at >= ? AND created_at < ?', @from, @to)
      @telephony_calls += TelephonyCall.where(agent_number: Personnel.find(@selected_executive).mobile, fresh: false, untagged: true).where('created_at >= ? AND created_at < ?', @from, @to)
    end
  ...
  def live_lead_whatsapp
    ...
    if params[:personnel_id] == nil && params[:data] == nil && current_personnel.email == "ayush@thejaingroup.com"
      ...
      if params[:data] == nil && params[:personnel_id] != nil
        ...
        if params[:personnel_id] == nil
          if params[:data] == nil
            @leads = []
            Lead.where(personnel_id: current_personnel.id, business_unit_id: 70, lost_reason_id: nil).each do |lead|
              @leads += [[lead.id, lead.name, lead.unseen_messages]]
            end
            @leads = @leads.sort_by{|_,_,x| x}.reverse
            @selected_lead = nil
