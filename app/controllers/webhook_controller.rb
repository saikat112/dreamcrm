...
skip_before_filter :verify_authenticity_token, only: [:livserve_data, :magic_bricks_data, :acres_data, :linkedin_data, :facebook_data, :adwords_data, :olx_data, :calls, :highrise_alcove_data, :google_form_data, :website_form_data, :check_lead_existence, :create_whatsapp, :fb_whatsapp_lead_creation, :alcove_chat, :rajat_chat, :mobile_check, :chat_id_check, :kiswok, :kiswok_incoming_message, :rajat_calls, :credai_data, :google_drive_call_record, :mark_lead_hot, :qualify_lead, :isv_lead, :ifttt_call_capture, :jsb_calls, :highrise_alcove_status_update, :highrise_alcove_transfer_update, :whatsapp_text_message, :whatsapp_media_message, :whatsapp_map_message, :webhook_chat, :super_receptionist, :jaingroup_website_form_data, :customer_whatsapp_reply_capture, :upload_transcript]
				...
				if broker_contacts == []
					...
					if leads == []
						...
						if leads.count == 1
							...
							if customer_message == "Schedule Site Visit" || customer_message == "Book Site Visit"
								UserMailer.site_visit_message_alert([leads[0]]).deliver
							end
						else
							...
							if live_lead == nil
								...
								UserMailer.lead_whatsapp(data).deliver
								if customer_message == "Schedule Site Visit" || customer_message == "Book Site Visit"
									UserMailer.site_visit_message_alert([live_lead, customer_message]).deliver
								end	
							end
						end
					end
	...
	def google_form_data
		apikey=params["google_key"]
		p apikey
		organisation=Organisation.find_by_api_key(apikey)
		p organisation.id
		mobile=params["user_column_data"][2]["string_value"]
		mobile=mobile.gsub(/\s+/, "")
		mobile=mobile.split(//).last(10).join
		p mobile
		name=params["user_column_data"][3]["string_value"]
		p name
		email=params["user_column_data"][1]["string_value"]
		p email
		customer_remarks=(params["user_column_data"][4]["string_value"])+","+(params["user_column_data"][5]["string_value"])
		p customer_remarks
		project=BusinessUnit.find_by_name('Aagaman')
		auto_lead=Lead.new
		auto_lead.generated_on=Time.now
		...
		auto_lead.email=email
		auto_lead.mobile=mobile
		auto_lead.source_category_id=6381
		auto_lead.business_unit_id=project.id
