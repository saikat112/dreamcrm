...
skip_before_filter :verify_authenticity_token, only: [:call_record_follow_up_entry_form, :personnel_wise_leads_genie]
...
def booked_leads
	...
	if current_personnel.status=='Team Lead'
		...
		if @sales_person==-1
			@leads=Lead.includes(:business_unit, :source_category, :personnel, :lost_reason, :follow_ups, :whatsapps).where(:personnels => {organisation_id: current_personnel.organisation_id},:leads => {business_unit_id: project_selected, status: true}).where('leads.booked_on >= ? AND leads.booked_on < ?', @from, ((@to.to_date)+1.day))
		else
...
def site_executive_options
  ...
  def customer_feedback_entry
  	...
  	lead.update(rating: params[:rating], customer_feedback: params[:site_visit_feedback])
